 ...
 *
 * Observabilité :
 * - Aucune (helper pur, sans logs).
 *
 * @layer lib/http
 * @remarks
 * - Ne sérialise pas automatiquement `init.body`. Si vous passez un objet, fournissez aussi l’en-tête `Content-Type: application/json` **et** `JSON.stringify(...)`.
 * - La détection d’erreur tente de lire un schéma `{"error": { code, message }}` ou un `{"message": string}` si présent.
 * - Si le corps n’est pas du JSON valide (ou vide), la fonction retourne `null`.
 */
