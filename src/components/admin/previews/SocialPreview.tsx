...
export function SocialInlinePreview({
  settings,
}: {
          ...
          items.map((it, i) => {
            ...
            return (
              ...
              >
                ...
                <span className="opacity-70">{socialKindLabel(it.kind, t)}</span>
              </li>
            );
          })
        ) : (
...
export function SocialPreview({ settings }: { settings: SocialSettingsDTO }) {
  const { t } = useI18n();
  return (
    <PreviewPanel
