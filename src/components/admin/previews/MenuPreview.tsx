...
export type MenuItemLikeProps = Readonly<{
  label: string;
  href: string;
  newTab?: boolean;
}>;
export type MenuLikeProps = Readonly<{
  items?: ReadonlyArray<MenuItemLikeProps> | null;
}>;

export function MenuInlinePreview({ settings }: { settings: MenuLikeProps }) {
  const { t } = useI18n();
    ...
    <nav aria-label={t("ui.preview.menu.ariaLabel")}>
      <ul className="flex flex-wrap gap-x-4 gap-y-3 text-sm">
        {items.length > 0 ? (
          ...
          <li className="opacity-60">{t("ui.preview.menu.empty")}</li>
        )}
      </ul>
    </nav>
  );
...
export function MenuPreview({ settings }: { settings: MenuLikeProps }) {
  const { t } = useI18n();
  return (
    <PreviewPanel label={t("admin.ui.preview.menu.label") || "Menu preview"}>
