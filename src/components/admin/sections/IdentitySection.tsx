...
export function IdentitySection() {
  ...
  const onPatchKV = useMemo(() => adaptPatchKV(patch), [patch]);
  ...
  const handleSubmit = useCallback(async () => {
    ...
    // 3) Normalisation minimale avant save (trim visibles)
    const title = settings.title.trim();
    const logoAlt = (settings.logoAlt ?? "").trim();
    if (title !== settings.title || logoAlt !== (settings.logoAlt ?? "")) {
      patch({ title, logoAlt });
    }

    // 4) Save + mapping erreurs 400 (Zod)
    try {
