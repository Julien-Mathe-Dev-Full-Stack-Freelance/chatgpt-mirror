...
export function ActionsBar({
  ...
  const { t } = useI18n();
  const resolvedSubmitLabel = submitLabel ?? t("ui.actions.submit");
  const resolvedSubmitLoadingLabel =
    submitLoadingLabel ?? t("ui.actions.saving");
  const resolvedResetLabel = resetLabel ?? t("ui.actions.reset");
  const resolvedAddLabel = addLabel ?? t("ui.actions.add");
      ...
      {loading ? (
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            >
              {saving ? resolvedSubmitLoadingLabel : resolvedSubmitLabel}
            </Button>
          </div>
        </div>
      ) : variant === "submitOnly" ? (
