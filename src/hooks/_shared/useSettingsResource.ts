...
import { isAbortError, newAbort } from "@/lib/abortError";
...
export type UseSettingsResourceParams<T> = Readonly<{
  state?: ContentState;
  defaults: T;
  entity: EntityKind;
  ...
  const save = useCallback(
    async (nextState?: ContentState) => {
      ...
      try {
        ...
        const candidate = (beforeSaveFn ? beforeSaveFn(settings) : settings) as T;
