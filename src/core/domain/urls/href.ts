...
export type RelativeUrl = string & Brand<"RelativeUrl">;
export type AbsoluteHttpUrl = string & Brand<"AbsoluteHttpUrl">;
export type MailtoUrl = string & Brand<"MailtoUrl">;
...
export function mail(s: string): MailtoUrl {
  if (process.env.NODE_ENV !== "production") {
    if (!isMailtoHref(s)) {
      throw new DomainError({
        code: ERROR_CODES.INTERNAL,
...
export function href(s: string): BrandedHref {
  const i = s.indexOf(":");
  if (i > 0) {
...
export function brandHrefSafe(
  ...
  return RELATIVE_URL_RE.test(s) ? (rel(s) as BrandedHref) : prev;
}
