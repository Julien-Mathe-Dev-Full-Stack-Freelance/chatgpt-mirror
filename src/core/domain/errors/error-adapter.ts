 ...
 * @intro Adaptation HTTP des erreurs domaine (règles par préfixe)
 * @layer infra/http
 * @remarks
 * - Ne JAMAIS exposer details/cause côté client.
 * - Corps minimal: { code } (optionnellement un message générique non sensible).
 */
...
const PREFIX_RULES: Array<{
  ...
  // ajoute ici d’autres familles le jour où tu en crées (ex: IMPORT_, SLUG_, etc.)
];
...
if (process.env.NODE_ENV !== "production") {
  ...
  if (misses.length) {
    console.warn(`[errors] No HTTP status rule for codes: ${misses.join(", ")}`);
  }
}
