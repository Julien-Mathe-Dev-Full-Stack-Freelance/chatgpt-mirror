...
export const DEFAULT_THEME_MODE: ThemeMode = "system" as const;
export const DEFAULT_THEME_PALETTE: ThemePalette = "neutral" as const;
...
if (process.env.NODE_ENV !== "production") {
  ...
  if (!inSet(DEFAULT_THEME_MODE, THEME_MODES as readonly string[])) {
    throw new DomainError({
      code: ERROR_CODES.INTERNAL,
      message: `[defaults/theme] themeMode inconnu: ${DEFAULT_THEME_MODE}`,
    });
  }
  if (!inSet(DEFAULT_THEME_PALETTE, PALETTES as readonly string[])) {
    throw new DomainError({
      code: ERROR_CODES.INTERNAL,
