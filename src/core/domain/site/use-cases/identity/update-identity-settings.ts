...
import { checkIdentityTitleRule } from "@/core/domain/site/validators/identity";
...
export function updateIdentitySettings({ repo }: UpdateIdentitySettingsDeps) {
  ...
  return async function run(
    ...
    return logWithDuration("uc.updateIdentitySettings", async () => {
      ...
      // 5) Règle métier : "titre requis si pas de logo" sur l’état final
      const ruleIssues = checkIdentityTitleRule(identityNext);
      if (ruleIssues.length > 0) {
        logger.warn("identity.rules.failed", { issues: ruleIssues });
        // On lève avec un code stable ; le message est volontairement concis côté domaine.
        throw new UpdateIdentitySettingsError(ruleIssues[0].code);
      }
