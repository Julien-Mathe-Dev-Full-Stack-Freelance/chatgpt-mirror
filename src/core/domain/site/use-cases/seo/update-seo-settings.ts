...
import { checkSeoRules } from "@/core/domain/site/seo/validators";
...
import { UpdateSeoSettingsError } from "@/core/domain/site/use-cases/seo/update-seo-settings.errors";
import type {
...
export function updateSeoSettings({ repo }: UpdateSeoSettingsDeps) {
  ...
  return async function run(
    ...
    return logWithDuration("uc.updateSeoSettings", async () => {
      ...
      // ✅ Règles SEO bloquantes minimales (voir §2)
      const issues = checkSeoRules(seoNext);
      if (issues.length > 0) {
        logger.warn("seo.rules.violation", { issues });
        throw new UpdateSeoSettingsError(issues[0].code);
      }
