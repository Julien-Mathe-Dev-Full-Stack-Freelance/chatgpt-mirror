/**
 * @file src/core/domain/site/use-cases/header/update-header-settings.types.ts
 * @intro Types du use-case `UpdateHeaderSettings`.
 * @description
...
export interface UpdateHeaderSettingsDeps {
  /** Dépôt agrégat “site” (index + settings). */
  repo: SiteRepository;
}

/** Données d’entrée : patch partiel + espace logique (draft par défaut en implémentation). */
export interface UpdateHeaderSettingsInput {
  /** Modification partielle appliquée au header. */
...
export interface UpdateHeaderSettingsResult {
  settings: SiteSettings;
}

// /** Runner type pour parité DI/tests */
// export type RunUpdateHeaderSettings = (
