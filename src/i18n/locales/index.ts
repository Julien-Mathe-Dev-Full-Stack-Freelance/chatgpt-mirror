...
import { BLOCK_TYPES, TEXT_ALIGN_X } from "@/core/domain/blocks/constants";
...
if (process.env.NODE_ENV !== "production") {
  ...
  for (const loc of SUPPORTED_LOCALES) {
    ...
    for (const tab of ADMIN_TABS) {
      const key = `admin.tabs.${tab}`;
      if (typeof peek(MESSAGES[loc], key) !== "string") {
        warn("i18n.missingLabel", { key, locale: loc });
      }
    }
    for (const act of ADMIN_ACTIONS) {
      const key = `admin.actions.${act}`;
      if (typeof peek(MESSAGES[loc], key) !== "string") {
        warn("i18n.missingLabel", { key, locale: loc });
      }
    }
    ...
    for (const w of SECTION_MAX_WIDTHS) {
      const key = `admin.layout.maxWidth.${w}`;
      if (typeof peek(MESSAGES[loc], key) !== "string") {
        warn("i18n.missingLabel", { key, locale: loc });
      }
    }
    for (const s of SECTION_SPACING_Y) {
      const key = `admin.layout.spacingY.${s}`;
      if (typeof peek(MESSAGES[loc], key) !== "string") {
        warn("i18n.missingLabel", { key, locale: loc });
      }
    }
    for (const a of SECTION_ALIGN_X) {
      const key = `admin.layout.align.${a}`;
      if (typeof peek(MESSAGES[loc], key) !== "string") {
        warn("i18n.missingLabel", { key, locale: loc });
      }
    }
    ...
    for (const a of TEXT_ALIGN_X) {
      const key = `admin.blocks.text.align.${a}`;
      if (typeof peek(MESSAGES[loc], key) !== "string") {
        warn("i18n.missingLabel", { key, locale: loc });
      }
    }
