...
import {
  UpdatePrimaryMenuSettingsSchema,
  type UpdatePrimaryMenuSettingsDTO,
...
export async function PATCH(req: Request) {
  ...
  return handleRoute(NS, req, async ({ state }) => {
    ...
    const dto: UpdatePrimaryMenuSettingsDTO = parseDTO(
      UpdatePrimaryMenuSettingsSchema,
      raw
    );
    ...
    const itemsLen =
      "items" in patch && Array.isArray(patch.items)
        ? patch.items.length
        : undefined;
